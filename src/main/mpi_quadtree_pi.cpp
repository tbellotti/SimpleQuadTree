...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0)  {
        ...
        if (min_level < level)  {
            ...
            min_level = static_cast<unsigned char>(level);
        }

        // This is the min level of the 
        // tree stored in each core
        min_level_core = static_cast<unsigned char>(min_level-level); 
        max_level_core = static_cast<unsigned char>(max_level-level);
